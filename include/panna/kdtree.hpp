...
private:
    ...
    size_t build_(size_t l, size_t r, int depth)
    {
        ...
        std::nth_element(indices_.begin() + l,
                         indices_.begin() + m,
                         indices_.begin() + r,
                         [&](uint32_t a, uint32_t b) {
                             return coord_(a, axis) < coord_(b, axis);
                         });
    ...
    float coord_(uint32_t idx, int d, float delta = 0.0f) const {
        return dataset_[idx].inner.chunks[d / Int16Chunk::CHUNK_SIZE]
            .chunk[d % Int16Chunk::CHUNK_SIZE] / 32768.0f + delta;
    }
    ...
    float min_dist_sq_(size_t a_idx, size_t b_idx) const {
        ...
        for (int d = 0; d < dim_; d++) {
            ...
            acc += diff * diff;
            if (acc > radius2_) break; // Early exit
        }
        return acc;
    }

    // Enumerate pairs of points within the radius
    void enumerate_(size_t a_idx, size_t b_idx, bool same,
    ...
    void maybe_emit_(uint32_t i, uint32_t j,
                     std::vector<std::tuple<float,
                                            std::pair<uint32_t, uint32_t>>>& out) const
    {
